...
def raster_to_point(raster_list, pop_shp, proj_path, crs, country):
    ...
    out_meta.update({"driver": "GTiff", "height": mosaic.shape[1],"width": mosaic.shape[2], "transform": out_trans, "crs": ({'init': crs})})
    ...
    if country == 'Benin':
        settlements = settlements[['VALUE', 'geometry']]
    else:
...
def raster_proximity(proj_path, files, country):
    ...
    if country == 'Benin':
        raster_list = [(os.path.join(proj_path, 'UTM31N_Benin Electricity Transmission Network.shp')),
                    (os.path.join(proj_path, 'UTM31N_benin roads.shp')),
                    (os.path.join(proj_path, 'UTM31N_ECOWAS_Electricity_Distribution.shp'))]
    else:
        raster_list = [os.path.join(proj_path,'Concat_Transmission_lines_UMT37S.shp'), os.path.join(proj_path,'UTM37S_Primary_Substations.shp'), os.path.join(proj_path,'UTM37S_Distribution_Transformers.shp'), os.path.join(proj_path,'Concat_Mini-grid_UMT37S.shp'),os.path.join(proj_path,'UTM37S_Roads.shp'), os.path.join(proj_path,'Concat_MV_lines_UMT37S.shp'), os.path.join(proj_path,'UTM37S_11kV.shp')]
    ...
    for i in range(len(raster_list)):
        ...
        RefImage = os.path.join(proj_path,files.loc['pop','filename'])
